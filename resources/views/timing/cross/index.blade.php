...
<style>
    ...
    .bg-gradient-cross { background: linear-gradient(135deg, #0f766e 0%, #0891b2 100%); }
    .jo-card {
        cursor: pointer; border: 2px solid #dee2e6; transition: all .22s; border-radius: .5rem;
    }
    .jo-card:hover:not(.jo-selected) {
        border-color: #0891b2; box-shadow: 0 3px 10px rgba(8,145,178,.15); transform: translateY(-1px);
    }
    .jo-card.jo-selected {
        border-color: #0f766e !important;
        background: linear-gradient(135deg,rgba(15,118,110,.1) 0%,rgba(8,145,178,.07) 100%);
        box-shadow: 0 4px 14px rgba(15,118,110,.25); transform: translateY(-2px);
    }
    .emp-card {
        cursor: pointer; border: 2px solid #dee2e6; transition: all .18s; border-radius: .5rem; user-select:none;
    }
    .emp-card:hover:not(.emp-selected):not(.emp-disabled) { border-color: #0891b2; background: #f0fdfe; }
    .emp-card.emp-selected {
        border-color: #0f766e !important;
        background: linear-gradient(135deg,rgba(15,118,110,.1) 0%,rgba(8,145,178,.07) 100%);
    }
    .emp-card.emp-disabled { opacity:.55; cursor:not-allowed; background:#f8f9fa; }
    .dept-chip, .pos-chip {
        border-radius: 20px !important; font-size: .68rem !important; padding: 2px 11px !important;
    }
    .dept-chip.active  { background:#0f766e!important; border-color:#0f766e!important; color:#fff!important; }
    .pos-chip.active   { background:#0891b2!important; border-color:#0891b2!important; color:#fff!important; }
    .session-card { transition:.15s; }
    .session-card:hover { background:#f0fdfe; }
    .session-card.selected-for-bulk { background:#fef2f2; }
    .duration-display { font-variant-numeric: tabular-nums; }
    #emp-grid::-webkit-scrollbar,#jo-list::-webkit-scrollbar{width:4px;}
    #emp-grid::-webkit-scrollbar-thumb,#jo-list::-webkit-scrollbar-thumb{background:#0891b2;border-radius:2px;}
</style>
@endpush
    ...
    <div class="d-flex align-items-center mb-3 gap-3 flex-wrap">
        <div>
            <h2 class="mb-0 fw-semibold" style="font-size:1.4rem;">
                ...
                <span style="background:linear-gradient(90deg,#0f766e,#0891b2);-webkit-background-clip:text;-webkit-text-fill-color:transparent;">Dept Cross Timing</span>
            </h2>
    ...
    <div class="row g-4">
                ...
                <div class="card-body pb-3">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-semibold small text-uppercase text-muted mb-1">
                            <span class="badge rounded-circle me-1" style="background:#0f766e;">1</span>Pilih Job Order
                        </label>
                        <input type="text" id="jo-search" class="form-control form-control-sm mb-2" placeholder="��� Cari nama job order atau project...">
                        <div id="jo-list" class="row g-1" style="max-height:220px;overflow-y:auto;padding:2px;">
                            @forelse($jobOrders as $jo)
                                ...
                                @endphp
                                <div class="col-12 jo-card-wrapper">
                                    <div class="jo-card p-2"
                                        data-jo-id="{{ $jo->id }}"
                                        data-jo-name="{{ strtolower($jo->name) }}"
                                        data-jo-project="{{ strtolower($jo->project->name ?? '') }}">
                                        <div class="d-flex align-items-center gap-2">
                                            <div class="flex-grow-1" style="min-width:0;">
                                                <div class="fw-semibold text-truncate" style="font-size:.82rem;line-height:1.3;">{{ $jo->name }}</div>
                                            ...
                                            <div class="flex-shrink-0">
                                                @if($daysLeft !== null)
                                                    @if($daysLeft < 0)
                                                        <span class="badge bg-danger" style="font-size:.6rem;"><i class="bi bi-exclamation-triangle-fill me-1"></i>OVERDUE {{ abs($daysLeft) }}d</span>
                                                    @elseif($daysLeft === 0)
                                                        <span class="badge bg-danger" style="font-size:.6rem;"><i class="bi bi-alarm-fill me-1"></i>DUE TODAY</span>
                                                    @elseif($daysLeft <= 3)
                                                        <span class="badge bg-warning text-dark" style="font-size:.6rem;"><i class="bi bi-hourglass-split me-1"></i>{{ $daysLeft }}d</span>
                                                    @else
                                                        <span class="badge bg-info text-dark" style="font-size:.6rem;">{{ $daysLeft }}d</span>
                                                    @endif
                                                @else
                                                    <span class="badge bg-secondary" style="font-size:.6rem;">No date</span>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-semibold small text-uppercase text-muted mb-1">
                            <span class="badge rounded-circle me-1" style="background:#0f766e;">2</span>Pilih Karyawan
                            <span class="text-muted fw-normal ms-1">({{ $employees->count() }} total)</span>
                        </label>

                        <div class="d-flex gap-1 mb-2">
                            <input type="text" id="emp-search" class="form-control form-control-sm flex-grow-1" placeholder="��� Cari nama / posisi / departemen...">
                            <button type="button" class="btn btn-outline-secondary btn-sm px-2" id="select-all-btn" title="Pilih Semua Terlihat"><i class="bi bi-check-all"></i></button>
                            <button type="button" class="btn btn-outline-secondary btn-sm px-2" id="deselect-all-btn" title="Batal Semua"><i class="bi bi-x-lg"></i></button>
                        </div>

                        {{-- Dept filter chips --}}
                        <div class="d-flex flex-wrap gap-1 mb-1" id="dept-filter-chips">
                            <button type="button" class="btn btn-sm dept-chip active" data-dept="all">Semua Dept</button>
                            @foreach($departments as $dept)
                                <button type="button" class="btn btn-sm btn-outline-secondary dept-chip" data-dept="{{ $dept->id }}">{{ $dept->name }}</button>
                            @endforeach
                        </div>

                        {{-- Position filter chips --}}
                        @if($positions->count() > 0)
                        <div class="d-flex flex-wrap gap-1 mb-2" id="pos-filter-chips">
                            <button type="button" class="btn btn-sm pos-chip active" data-pos="all">Semua Posisi</button>
                            @foreach($positions as $pos)
                                <button type="button" class="btn btn-sm btn-outline-info pos-chip" data-pos="{{ strtolower($pos) }}">{{ $pos }}</button>
                            @endforeach
                        </div>
                        @endif

                        {{-- Employee Grid --}}
                        <div id="emp-grid" class="row g-1" style="max-height:250px;overflow-y:auto;padding:2px;">
                            @foreach($employees as $emp)
                                @php $hasActive = in_array($emp->id, $employeesWithActiveSessions); @endphp
                                <div class="col-6 col-sm-4 emp-card-wrapper"
                                    data-emp-id="{{ $emp->id }}"
                                    ...
                                    <div class="emp-card p-2 text-center h-100 position-relative {{ $hasActive ? 'emp-disabled' : '' }}" style="min-height:72px;">
                                        <input type="checkbox" class="emp-checkbox d-none" id="emp-{{ $emp->id }}" value="{{ $emp->id }}" {{ $hasActive ? 'disabled' : '' }}>
                                        @if($hasActive)
                                            <span class="position-absolute top-0 end-0 m-1 badge bg-warning text-dark" style="font-size:.55rem;">
                                                <i class="bi bi-circle-fill me-1" style="font-size:.45rem;"></i>Running
                                            </span>
                                        @endif
                                        @if($emp->photo)
                                            <img src="{{ asset('storage/'.$emp->photo) }}" class="rounded-circle mb-1 border" width="38" height="38" style="object-fit:cover;">
                                        @else
                                            <div class="rounded-circle bg-secondary d-inline-flex align-items-center justify-content-center mb-1" style="width:38px;height:38px;">
                                                <i class="bi bi-person text-white" style="font-size:.9rem;"></i>
                                            </div>
                                        @endif
                                        <div class="fw-semibold lh-sm text-truncate" style="font-size:.72rem;">{{ $emp->name }}</div>
                                        <div class="text-muted text-truncate" style="font-size:.62rem;">{{ $emp->position ?? '-' }}</div>
                                        @if($emp->department)
                                            <div class="text-truncate" style="font-size:.58rem;color:#0891b2;">{{ $emp->department->name }}</div>
                                        @endif
                                    </div>
                                </div>
                            @endforeach
                        ...
                        <div class="mt-1 d-flex align-items-center gap-2 flex-wrap">
                            <small class="text-muted"><span id="selected-count">0</span> karyawan dipilih <span id="filtered-count" class="ms-1"></span></small>
                            <span id="active-warn" class="text-warning d-none small"><i class="bi bi-exclamation-triangle-fill me-1"></i>Beberapa sudah running</span>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select id="parts-select" class="form-select form-select-sm" required>
                            <option value="">— Pilih parts —</option>
                    ...
                    <div class="row g-2 mb-3">
                        <div class="col-7">
                            <label class="form-label small fw-semibold text-muted mb-1">
                                <span class="badge rounded-circle me-1" style="background:#0f766e;">4</span>Task <span class="fw-normal">(opsional)</span>
                            </label>
                            <input type="text" id="task-input" class="form-control form-control-sm" placeholder="e.g., Sewing, Airbrush...">
                        </div>
                        <div class="col-5">
        ...
        <div class="col-lg-7">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-gradient-cross text-white d-flex justify-content-between align-items-center py-2">
                    <h5 class="mb-0">
                        <i class="bi bi-clock-history me-2"></i>Sesi Aktif
                        <span class="badge bg-white text-dark ms-2" id="active-count">{{ $activeSessions->count() }}</span>
                    </h5>
                    <div class="d-flex gap-2">
                        <button class="btn btn-sm btn-light" id="refresh-sessions-btn" title="Refresh"><i class="bi bi-arrow-clockwise"></i></button>
                        <button class="btn btn-sm btn-light text-danger fw-bold" id="bulk-stop-btn" disabled><i class="bi bi-stop-circle me-1"></i>Bulk Stop</button>
                    </div>
                </div>
                ...
                <div id="sessions-container" style="max-height:calc(100vh - 240px);overflow-y:auto;">
                    @if($activeSessions->isEmpty())
                        <div class="text-center py-5 text-muted">
                            <i class="bi bi-shuffle" style="font-size:2.5rem;opacity:.25;display:block;margin-bottom:.5rem;"></i>
                    ...
                    @else
                        @foreach($activeSessions as $session)
                            @php
                                ...
                                $cardColor = $isFrozen ? '#f59e0b' : ($isRepair ? '#fd7e14' : '#0f766e');
                            @endphp
                            <div class="session-card p-3 border-bottom" id="session-card-{{ $session->id }}" data-timing-id="{{ $session->id }}" style="border-left:4px solid {{ $cardColor }};">
                                <div class="d-flex align-items-start gap-2">
                                    <input type="checkbox" class="form-check-input bulk-session-cb flex-shrink-0 mt-1" value="{{ $session->id }}">
                                    <div class="flex-grow-1" style="min-width:0;">
                                        <div class="d-flex align-items-center gap-1 flex-wrap mb-1">
                                            <span class="fw-semibold" style="font-size:.9rem;">{{ $session->employee->name ?? '-' }}</span>
                                            @if($isFrozen)
                                            ...
                                            @else
                                                <span class="badge" style="background:#0f766e;font-size:.62rem;"><i class="bi bi-shuffle me-1"></i>CROSS</span>
                                            @endif
                                        </div>
                                        ...
                                        <div class="text-muted" style="font-size:.72rem;">
                                            <i class="bi bi-tools me-1"></i>{{ $session->step ?: '—' }}
                                            @if($session->parts) &bull; <i class="bi bi-box me-1"></i>{{ $session->parts }} @endif
                                            &bull; <i class="bi bi-clock me-1"></i>{{ $session->start_time ? \Carbon\Carbon::parse($session->start_time)->format('H:i') : '-' }}
                                        </div>
                                        <div class="mt-1">
                                            @if($isFrozen)
                                    ...
                                    <div class="d-flex flex-column gap-1 flex-shrink-0">
                                        @if($isFrozen)
                                            <button class="btn btn-success btn-sm unfreeze-btn" data-timing-id="{{ $session->id }}" data-employee-name="{{ $session->employee->name ?? '' }}" style="font-size:.72rem;padding:3px 10px;"><i class="bi bi-play-fill me-1"></i>Lanjut</button>
                                        @else
                                            <button class="btn btn-warning btn-sm freeze-btn" data-timing-id="{{ $session->id }}" data-employee-name="{{ $session->employee->name ?? '' }}" style="font-size:.72rem;padding:3px 10px;"><i class="bi bi-pause-fill me-1"></i>Pause</button>
                                        @endif
        ...
        <div class="modal-content">
            ...
            <form id="stop-work-form">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-bold">Select Stage Completed <span class="text-danger">*</span></label>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn fw-bold text-white" id="stop-submit-btn" style="background:linear-gradient(135deg,#0f766e,#0891b2);">
...
$(function() {
    ...
    $('#jo-search').on('input', function() {
        ...
        $('#jo-list .jo-card-wrapper').each(function() {
            ...
            const show = !q || (card.data('jo-name')||'').includes(q) || (card.data('jo-project')||'').includes(q);
            $(this).toggle(show);
        });
    });
    ...
    $(document).on('click', '.dept-chip', function() {
        $('.dept-chip').removeClass('active').addClass('btn-outline-secondary').css({'background':'','border-color':'','color':''});
        $(this).removeClass('btn-outline-secondary').addClass('active').css({'background':'#0f766e','border-color':'#0f766e','color':'#fff'});
        activeDept = String($(this).data('dept'));
        applyEmployeeFilters();
    });

    /* ══ POSITION FILTER ══ */
    $(document).on('click', '.pos-chip', function() {
        $('.pos-chip').removeClass('active').addClass('btn-outline-info').css({'background':'','border-color':'','color':''});
        $(this).removeClass('btn-outline-info').addClass('active').css({'background':'#0891b2','border-color':'#0891b2','color':'#fff'});
        activePos = String($(this).data('pos'));
        applyEmployeeFilters();
    });
    ...
    function applyEmployeeFilters() {
        ...
        $('.emp-card-wrapper').each(function() {
            ...
            const posOk  = activePos  === 'all' || ($(this).data('emp-position')||'').includes(activePos);
            const show   = nameOk && deptOk && posOk;
            $(this).toggle(show);
            if (show) visible++;
        });
        const isFiltered = activeDept !== 'all' || activePos !== 'all' || q;
        $('#filtered-count').html(isFiltered ? '<span class="badge" style="background:#0891b2;">'+visible+' tampil</span>' : '');
    }

    /* ══ EMP SELECT (click card) ══ */
    $(document).on('click', '.emp-card-wrapper .emp-card:not(.emp-disabled)', function() {
    ...
    $('#select-all-btn').on('click', function() {
        $('.emp-card-wrapper:visible .emp-checkbox:not(:disabled)').prop('checked', true).trigger('change');
    });
    ...
    function updateSelectedCount() {
        ...
        selectedEmployees.forEach(id => {
            if ($('.emp-card-wrapper[data-emp-id="'+id+'"]').attr('data-has-active') === 'true') hasActive = true;
        });
    ...
    function updateStartBtn() {
        ...
        const ok = selectedJoId && selectedEmployees.size > 0 && parts !== '';
    ...
    $('#start-btn').on('click', function() {
        ...
        const parts = ($('#parts-select').val() || '').trim();
        if (!parts) { $('#parts-error').removeClass('d-none'); $('#parts-select').focus(); return; }
        ...
        $.ajax({
            ...
            data: { _token:CSRF, employees:employeeArr, job_order_id:selectedJoId,
                    task, tasks:tasksPayload, session_type:sessionType, session_types:sessionTypesPayload, parts },
            success(res) {
                if (res.success) {
                    ...
                    $('#task-input').val(''); $('#parts-select').val('');
                    updateSelectedCount(); updateStartBtn();
                }
            },
            error(xhr) { Swal.fire({ icon:'error', title:'Error', text:xhr.responseJSON?.message||'Failed to start.' }); },
            complete() {
                btn.prop('disabled', false).html('<i class="bi bi-play-circle-fill me-2"></i><span id="start-btn-text">START WORK</span>');
                updateStartBtn();
            },
        });
    });
    ...
    $('#stop-stage').on('change', function() {
        ...
        $('#current-progress-display').text(stage >= 1 && stage <= 10 ? stage * 10 : currentPrevProgress);
    });
    ...
    $('#stop-work-form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            method: 'POST',
            data: { _token:CSRF, timing_id:$('#stop-timing-id').val(), stage,
                    output_qty:$('#stop-output-qty').val(), measurement_type:$('#stop-measurement-type').val() },
            success(res) {
                if (res.success) {
            ...
            error(xhr) { Swal.fire({ icon:'error', title:'Error', text:xhr.responseJSON?.message||'Failed to stop.' }); },
            complete() { btn.prop('disabled', false).html('<i class="bi bi-stop-circle me-1"></i>Stop &amp; Save'); },
        });
    });

    /* ══ FREEZE ══ */
    $(document).on('click', '.freeze-btn', function() {
        const timingId = $(this).data('timing-id'), name = $(this).data('employee-name');
        Swal.fire({ icon:'info', title:'Pause Session?', html:'Timer <strong>'+name+'</strong> akan di-pause.',
            showCancelButton:true, confirmButtonColor:'#ffc107', confirmButtonText:'<i class="bi bi-pause-circle"></i> Pause', cancelButtonText:'Batal'
        }).then(r => {
            if (!r.isConfirmed) return;
            $.ajax({ url:'{{ route('timing-cross.freeze') }}', method:'POST', data:{_token:CSRF, timing_id:timingId},
                success(res) { if(res.success){Swal.fire({icon:'success',title:'Paused!',text:res.message,timer:1500,showConfirmButton:false});loadActiveSessions();}else Swal.fire({icon:'error',title:'Error',text:res.message}); },
    ...
    $(document).on('click', '.unfreeze-btn', function() {
        const timingId = $(this).data('timing-id'), name = $(this).data('employee-name');
        Swal.fire({ icon:'question', title:'Lanjutkan Session?', html:'Timer <strong>'+name+'</strong> akan dilanjutkan.',
            showCancelButton:true, confirmButtonColor:'#0f766e', confirmButtonText:'<i class="bi bi-play-circle"></i> Lanjut', cancelButtonText:'Batal'
        }).then(r => {
            if (!r.isConfirmed) return;
            $.ajax({ url:'{{ route('timing-cross.unfreeze') }}', method:'POST', data:{_token:CSRF, timing_id:timingId},
                success(res) { if(res.success){Swal.fire({icon:'success',title:'Dilanjutkan!',text:res.message,timer:1500,showConfirmButton:false});loadActiveSessions();}else Swal.fire({icon:'error',title:'Error',text:res.message}); },
                error(xhr){ Swal.fire({icon:'error',title:'Error',text:xhr.responseJSON?.message||'Failed.'}); }
            });
        });
    });
        ...
        $.ajax({
            ...
            success(res) {
                ...
                if (res.sessions.length === 0) {
                    container.html('<div class="text-center py-5 text-muted"><i class="bi bi-shuffle" style="font-size:2.5rem;opacity:.25;display:block;margin-bottom:.5rem;"></i>Belum ada sesi aktif hari ini.</div>');
    ...
    function buildSessionCard(s) {
        const isFrozen = s.status === 'frozen', isRepair = s.session_type === 'repair';
        const cc = isFrozen ? '#f59e0b' : (isRepair ? '#fd7e14' : '#0f766e');
        const badge = isFrozen
            ? '<span class="badge bg-warning text-dark" style="font-size:.62rem;"><i class="bi bi-pause-circle me-1"></i>PAUSED</span>'
            : (isRepair ? '<span class="badge" style="background:#fd7e14;font-size:.62rem;"><i class="bi bi-tools me-1"></i>REPAIR</span>'
                        : '<span class="badge" style="background:#0f766e;font-size:.62rem;"><i class="bi bi-shuffle me-1"></i>CROSS</span>');
        const timer = isFrozen
            ? '<span class="fw-bold text-warning" style="font-size:.9rem;">'+(s.frozen_duration||'00:00:00')+'</span>'
            : '<span class="duration-display fw-bold" style="font-size:.9rem;color:'+cc+';" data-start-time="'+s.start_time+'">00:00:00</span>';
        const actBtn = isFrozen
        ...
        const partsHtml = s.parts ? ' &bull; <i class="bi bi-box me-1"></i>'+s.parts : '';
        return '<div class="session-card p-3 border-bottom" id="session-card-'+s.id+'" data-timing-id="'+s.id+'" style="border-left:4px solid '+cc+';">'
            +'<div class="d-flex align-items-start gap-2">'
            +'<input type="checkbox" class="form-check-input bulk-session-cb flex-shrink-0 mt-1" value="'+s.id+'">'
            +'<div class="flex-grow-1" style="min-width:0;">'
            +'<div class="d-flex align-items-center gap-1 flex-wrap mb-1"><span class="fw-semibold" style="font-size:.9rem;">'+s.employee_name+'</span>'+badge+'</div>'
            ...
            +'<div class="text-muted" style="font-size:.72rem;"><i class="bi bi-tools me-1"></i>'+(s.task||'—')+partsHtml+' &bull; <i class="bi bi-clock me-1"></i>'+(s.start_time?s.start_time.slice(0,5):'-')+'</div>'
            +'<div class="mt-1">'+timer+'</div>'
            +'</div>'
            +'<div class="d-flex flex-column gap-1 flex-shrink-0">'
            +actBtn
    ...
    function startDurationTimers() {
        ...
        durationInterval = setInterval(() => {
            $('.duration-display').each(function() {
                const st = $(this).data('start-time'); if (!st) return;
                const now = new Date();
    ...
    $(document).on('change', '.bulk-session-cb', function() {
        ...
        if ($(this).is(':checked')) { selectedBulkIds.add(id); $(this).closest('.session-card').addClass('selected-for-bulk'); }
        else { selectedBulkIds.delete(id); $(this).closest('.session-card').removeClass('selected-for-bulk'); }
    ...
    $('#confirm-bulk-stop-btn').on('click', function() {
        ...
        const btn = $(this).prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-2"></span>Stopping...');
        $.ajax({
            url: '{{ route('timing-cross.bulk-stop') }}', method:'POST',
            data: { _token:CSRF, timing_ids:Array.from(selectedBulkIds), output_qty:$('#bulk-output-qty').val(), measurement_type:$('#bulk-measurement-type').val() },
            success(res) {
                if (res.success) {
                    Swal.fire({icon:'success',title:'Done!',text:res.message,timer:1500,showConfirmButton:false});
                    selectedBulkIds.clear(); $('#bulk-stop-panel').addClass('d-none'); $('#bulk-stop-btn').prop('disabled',true);
                    loadActiveSessions();
                }
            },
            error(xhr){ Swal.fire({icon:'error',title:'Error',text:xhr.responseJSON?.message||'Failed.'}); },
