            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    <form id="filter-form" class="row g-1">
                        ...
                        <div class="col-md-2">
                            <select id="returned_by_filter" class="form-select form-select-sm select2">
                        ...
                        <div class="col-md-2">
                            <input type="text" id="returned_at_filter" class="form-control form-control-sm"
                                placeholder="Returned At Date" autocomplete="off">
                        </div>
                        <div class="col-md-3">
    ...
    <script>
        $(document).ready(function() {
            ...
            const table = $('#datatable').DataTable({
                ...
                stateSave: true,
                ajax: {
                    url: "{{ route('goods_in.index') }}",
                    data: function(d) {
                        d.material_filter = $('#material_filter').val();
                        d.project_filter = $('#project_filter').val();
                        d.returned_by_filter = $('#returned_by_filter').val();
                        d.returned_at_filter = $('#returned_at_filter').val();
                ...
                language: {
                    emptyTable: '<div class="text-muted py-2">No goods in data available</div>',
            ...
            $('#material_filter, #project_filter, #returned_by_filter').on('change', function() {
                table.ajax.reload();
            });

            $('#returned_at_filter').on('change', function() {
                table.ajax.reload();
            });
            ...
            $('#reset-filters').on('click', function() {
                $('#material_filter, #project_filter, #returned_by_filter').val('').trigger('change');
                $('#returned_at_filter').val('');
                $('#custom-search').val('');
                table.ajax.reload();
            });
            ...
            // Initialize flatpickr for returned at date filter
            if (window.flatpickr) {
                flatpickr("#returned_at_filter", {
                    dateFormat: "Y-m-d",
                    allowInput: true,
                    locale: "default"
                });
                ...
                const dateInput = document.getElementById('returned_at_filter');
                if (dateInput) {
            ...
            $(document).on('click', '.btn-delete', function() {
                ...
                Swal.fire({
                    title: 'Are you sure?',
                    text: `You want to delete Goods In for "${material}"?`,
                    icon: 'warning',
