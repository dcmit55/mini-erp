            ...
            <form action="{{ route('material_requests.update', $materialRequest->id) }}" method="POST">
                        ...
                        <div class="d-flex gap-4">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="project_type" id="projectTypeClient"
                                       value="client" {{ old('project_type', $materialRequest->project_type) == 'client' ? 'checked' : '' }}>
                            ...
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="project_type" id="projectTypeInternal"
                                       value="internal" {{ old('project_type', $materialRequest->project_type) == 'internal' ? 'checked' : '' }}>
                ...
                <div class="row">
                    ...
                    <div class="col-lg-6 mb-3">
                        <div class="d-flex justify-content-between align-items-center mb-1">
                            ...
                            <button type="button" class="btn btn-sm btn-outline-primary {{ $materialRequest->project_type == 'client' ? 'd-none' : '' }}" id="btnAddInternalProject">
                        ...
                        <div id="projectInfoDisplay" class="mt-1 p-2 bg-light rounded {{ $materialRequest->project_id || $materialRequest->internal_project_id ? '' : 'd-none' }}">
                            ...
                            <input type="hidden" name="project_id" id="hiddenProjectId" value="{{ $materialRequest->project_id }}">
                            <input type="hidden" name="internal_project_id" id="hiddenInternalProjectId" value="{{ $materialRequest->internal_project_id }}">
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-lg-6 mb-3">
                        ...
                        <input type="text" class="form-control" value="{{ ucfirst($materialRequest->requested_by) }}" disabled>
                    </div>
                ...
                <div class="row">
                    <div class="col-lg-12 mb-3">
                        ...
                        <select name="status" class="form-select"
                            ...
                            <option value="pending" {{ $materialRequest->status === 'pending' ? 'selected' : '' }}>Pending</option>
                            <option value="approved" {{ $materialRequest->status === 'approved' ? 'selected' : '' }}>Approved</option>
                            <option value="canceled" {{ $materialRequest->status === 'canceled' ? 'selected' : '' }}>Canceled</option>
                        </select>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" id="project_qty" name="qty" min="0" step="any">
                    </div>
                    <div class="mb-3">
                        <label for="project_departments" class="form-label">Department <span class="text-danger">*</span></label>
                        <select name="department_ids[]" id="project_departments" class="form-select select2" multiple required>
                            @foreach ($departments as $dept)
                                <option value="{{ $dept->id }}">{{ $dept->name }}</option>
                            @endforeach
                        </select>
...
$(document).ready(function() {
    ...
    const currentJobOrderId = '{{ $materialRequest->job_order_id ?? $materialRequest->internal_project_id }}';
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success && response.project) {
                    ...
                    Swal.fire('Success', 'Project added successfully. Please refresh the page.', 'success')
                        .then(() => {
