...
<div class="container-fluid mt-4">
    ...
    <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between mb-3 gap-3">
        ...
        <div class="d-flex gap-2 flex-wrap align-items-center">
            <form action="{{ route('job-orders.gallery.index') }}" method="GET" class="d-flex gap-2" id="filterForm">
                <select name="project" class="form-select form-select-sm select2" onchange="this.form.submit()" style="min-width:180px;">
    ...
    <div class="d-flex gap-2 mb-4 border-bottom pb-2">
        <button class="btn btn-sm btn-purple rounded-pill px-3 tab-btn active-tab" id="btn-wip" onclick="switchTab('wip')">
            <i class="bi bi-camera-fill me-1"></i> WIP Photos
            <span class="badge bg-white text-purple ms-1">{{ $wipJobs->count() }}</span>
        </button>
        <button class="btn btn-sm btn-outline-purple rounded-pill px-3 tab-btn" id="btn-design" onclick="switchTab('design')">
            ...
            @foreach($designJobs as $jo)
                @php
                    $designPhotos = array_merge(
                        array_map(fn($p) => ['path' => $p, 'type' => 'Project Image',  'label' => 'project'], $jo->project_images  ?? []),
                        array_map(fn($p) => ['path' => $p, 'type' => 'Latest Design',  'label' => 'design'],  $jo->latest_designs  ?? []),
                        array_map(fn($p) => ['path' => $p, 'type' => 'Final Image',    'label' => 'final'],   $jo->final_images    ?? []),
                    );
                @endphp
                @if(count($designPhotos) > 0)
                    ...
                    <div class="d-flex align-items-center gap-2 mb-2 mt-4">
                        ...
                        @if($jo->status)
                            <span class="badge rounded-pill x-small status-badge-{{ Str::slug($jo->status) }}">{{ $jo->status }}</span>
                        @endif
                    </div>
                        ...
                        @foreach($designPhotos as $idx => $photo)
                            ...
                            <div class="col-6 col-sm-4 col-md-3 col-xl-2">
                                <div class="gallery-card shadow-sm rounded-3 overflow-hidden">
                                    <div class="gallery-photo-wrap">
                                        <a href="{{ $url }}"
                                           data-fancybox="design-{{ $jo->id }}"
                                           data-caption="<strong>{{ e($jo->name) }}</strong><br><span class='text-muted small'>{{ e($photo['type']) }} | {{ e($jo->project->name ?? 'No Project') }}</span>@if($jo->description)<br><p class='mt-2 mb-0'>{{ e(Str::limit($jo->description, 120)) }}</p>@endif">
                                            <img src="{{ $url }}"
                                                 class="gallery-photo"
                                                 alt="{{ $jo->name }}"
                                                 loading="lazy">
                                            <div class="gallery-overlay">
                                                <span class="badge bg-light text-dark x-small">{{ $photo['type'] }}</span>
                                            </div>
                                        </a>
                                    </div>
                                    <div class="gallery-desc p-2">
...
<style>
    .btn-purple         { background-color: #4A25AA; border-color: #4A25AA; color: #fff; }
    .btn-purple:hover   { background-color: #3a1d88; color: #fff; }
    .btn-outline-purple { border-color: #4A25AA; color: #4A25AA; }
    .btn-outline-purple:hover { background-color: #4A25AA; color: #fff; }
    .bg-purple          { background-color: #4A25AA !important; }
    .text-purple        { color: #4A25AA !important; }
    ...
    .gallery-desc { border-top: 1px solid #f0f0f0; }
    .desc-clamp {
        display: -webkit-box;
        -webkit-line-clamp: 2;
    ...
    .x-small { font-size: .68rem; }

    /* Status badge colours */
    .status-badge-on-progress { background:#d1ecf1;color:#0c5460; }
    .status-badge-delivered    { background:#d4edda;color:#155724; }
    .status-badge-pending      { background:#fff3cd;color:#856404; }
    .status-badge-cancelled    { background:#f8d7da;color:#721c24; }

    [data-bs-theme="dark"] .gallery-card { background: #1e1e2d; }
    [data-bs-theme="dark"] .gallery-desc { border-color: #2d2d3d; }
</style>
...
<script>
    ...
    function switchTab(tab) {
        ...
        document.getElementById('btn-wip').className    = 'btn btn-sm rounded-pill px-3 tab-btn ' + (tab === 'wip'    ? 'btn-purple active-tab' : 'btn-outline-purple');
        document.getElementById('btn-design').className = 'btn btn-sm rounded-pill px-3 tab-btn ' + (tab === 'design' ? 'btn-purple active-tab' : 'btn-outline-purple');
    }
</script>
@endpush
