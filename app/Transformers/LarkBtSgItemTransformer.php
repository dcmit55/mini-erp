...
/**
 * Lark BT-SG Item Transformer
 */
class LarkBtSgItemTransformer
{
    public function transform(LarkBtSgItemDTO $dto): array
    {
    ...
    /**
     * Match courier name with lark_bt_sg_courier_ids table
    ...
    private function matchCourier(?string $courierName): ?int
    {
        ...
        $courier = LarkBtSgCourierId::where('name', $courierName)->first();
        ...
        // Try case-insensitive match
        $courier = LarkBtSgCourierId::whereRaw('LOWER(name) = ?', [strtolower($courierName)])->first();
